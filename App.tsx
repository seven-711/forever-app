...
// UPDATED IMPORT: Using OSM for search/location to avoid 429 quota errors on Gemini
import { suggestLocationName, searchPlaces } from './services/osmService'; 
...
const App: React.FC = () => {
  ...
  // Debounced Search Effect using OSM (Nominatim)
  useEffect(() => {
    // Debounce reduced to 800ms since OSM is faster and free
  ...
  const handleCreateNote = async (text: string, authorName: string, imageUrl?: string) => {
    ...
    try {
      // Use OSM for location naming (Free/Unlimited)
    ...
    <div className="relative w-screen h-screen overflow-hidden flex flex-col md:flex-row bg-transparent font-sans">
      ...
      */}
      ...
      {!selectedLocation && !isInitialLoading && (
        <div className="absolute bottom-[calc(1.5rem+env(safe-area-inset-bottom))] md:bottom-8 left-1/2 -translate-x-1/2 z-[500] pointer-events-auto">
            <button
      ...
      {!selectedLocation && !isInitialLoading && notes.length > 0 && (
        <div className="absolute bottom-[calc(1.5rem+env(safe-area-inset-bottom))] md:bottom-8 right-[calc(1rem+env(safe-area-inset-right))] md:right-8 z-[500] flex flex-col items-end gap-3 pointer-events-none">
