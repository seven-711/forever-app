  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden flex flex-col md:flex-row bg-transparent font-sans">
      ...
      */}
      ...
      {/* Mobile Button Container - Bottom Center Layout */}
      <div className="md:hidden fixed bottom-[calc(2rem+env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 z-[500] flex items-end gap-5 pointer-events-none px-4 pb-2">
        ...
        {!selectedLocation && !isInitialLoading && (
          <div className="pointer-events-auto mb-1">
            <button
              ...
              className={`flex items-center justify-center w-16 h-16 rounded-full shadow-2xl shadow-purple-900/40 hover:scale-105 active:scale-95 transition-all group ring-4 ring-white/40 text-white ${isAdmin ? 'bg-gradient-to-r from-amber-500 to-orange-600' : 'bg-gradient-to-r from-purple-600 to-indigo-600'}`}
            >
              {isAdmin ? <ShieldCheck size={28} className="fill-white/20" /> : <PenTool size={28} className="fill-white/20" />}
            </button>
          </div>
        )}
              ...
              <div className="flex gap-1">
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap flex-1 justify-center relative overflow-hidden ${activeInfoTab === tab.id
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-4 gap-2">
                        {[
                          ...
                          <button
                            key={cat.id}
                            className={`flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl border transition-all duration-200 ${activeInfoTab === cat.id
